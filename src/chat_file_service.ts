...
export class ChatFileService {
    ...
    public async download(file: ChatFileMetadata, messageService: ChatMessageService): Promise<void> {
        const dirname = path.dirname(file.path);
        const filename = path.basename(file.path);
        const targetPath = path.join(this.rootPath, `${file.ip}_${file.port}`, dirname, filename);
